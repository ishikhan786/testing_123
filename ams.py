# from fastapi import FastAPI, Query, HTTPException
# from fastapi.middleware.cors import CORSMiddleware
# from pymongo import MongoClient
# from typing import List, Optional
# from pydantic import BaseModel, Field
# import uvicorn
# from fastapi import FastAPI, Query, HTTPException
# from fastapi.responses import JSONResponse
# from pydantic import BaseModel
# from typing import List, Optional
# from datetime import datetime
# from dateutil.parser import parse as parse_date, ParserError
# from rapidfuzz import fuzz
# from pymongo import MongoClient


# try:
#     client = MongoClient("mongodb://aml_eximbank_user:aksa%40%231234@10.0.0.83:27017/CFT?authSource=AML_EXIM_BANK")
#     db = client["AML_EXIM_BANK"]
#     collection = db["Adverse_media_4"]
# except Exception as e:
#     raise Exception(f"MongoDB connection failed: {e}")

# app = FastAPI()

# FUZZY_MATCH_THRESHOLD = 70


# class RiskAssessment(BaseModel):
#     name: str
#     status: str
#     confidence: Optional[int] = None
#     justification: Optional[List[str]] = None


# class PersonRecord(BaseModel):
#     url: str
#     title: str
#     description: str
#     date: Optional[str]
#     persons: List[str]
#     organizations: List[str]
#     risk_assessment: Optional[List[RiskAssessment]]


# def is_fuzzy_match(query_name: str, target_name: str, threshold: int = FUZZY_MATCH_THRESHOLD) -> bool:
#     query_tokens = set(query_name.lower().strip().split())
#     target_tokens = set(target_name.lower().strip().split())

#     # Exact token subset match
#     if query_tokens.issubset(target_tokens):
#         return True

#     # Different fuzzy matching strategies
#     score_partial = fuzz.partial_ratio(query_name, target_name)
#     score_token_sort = fuzz.token_sort_ratio(query_name, target_name)
#     score_token_set = fuzz.token_set_ratio(query_name, target_name)
#     score_reverse = fuzz.ratio(query_name[::-1], target_name[::-1])
#     score_substring = 100 if query_name in target_name or target_name in query_name else 0

#     return max(score_partial, score_token_sort, score_token_set, score_reverse, score_substring) >= threshold


# def filter_person_data_only(article, target_name: str):
#     """
#     Keep only relevant persons/organizations from article that match target_name.
#     """
#     persons = article.get("persons", [])
#     organizations = article.get("organizations", [])

#     matched_persons = [p for p in persons if is_fuzzy_match(target_name, p)]
#     matched_orgs = [o for o in organizations if is_fuzzy_match(target_name, o)]

#     article["persons"] = matched_persons
#     article["organizations"] = matched_orgs

#     return article


# @app.get("/person")
# def get_person_articles(name: str = Query(...)):
#     raw_results = list(collection.find({
#         "$or": [
#             {"persons": {"$elemMatch": {"$regex": f"^{name}$", "$options": "i"}}},
#             {"organizations": {"$elemMatch": {"$regex": f"^{name}$", "$options": "i"}}}
#         ]
#     }, {
#         "_id": 0,
#         "url": 1,
#         "title": 1,
#         "date": 1,
#         # "persons": 1,
#         # "organizations": 1,
#         # "description": 1,
#         # "risk_assessment": 1
#     }))

#     # if not raw_results:
#     #     raise HTTPException(status_code=404, detail=f"No articles found for '{name}'")

#     processed_results = []
#     for record in raw_results:
#         try:
#             dt = parse_date(record.get("date", ""))
#             record["parsed_date"] = dt.replace(tzinfo=None)
#         except (TypeError, ValueError, ParserError):
#             record["parsed_date"] = datetime.min
#         processed_results.append(record)

#     # Sort by parsed_date descending
#     processed_results.sort(key=lambda x: x["parsed_date"], reverse=True)

#     # Filter out only matching persons/organizations
#     for article in processed_results:
#         # article = filter_person_data_only(article, name)
#         article.pop("parsed_date", None)


#     # Prepare response metadata
#     total = len(processed_results)
#     if total > 0:
#         ResponseStatus = "MATCHES"
#         response = {
#             "ResponseStatus": ResponseStatus,
#             "articles": processed_results
#         }
#     else:
#         ResponseStatus = "CLEAR"
#         response = {
#             "ResponseStatus": ResponseStatus,
#             "details": f"No data found for {name}",
#         }

#     return JSONResponse(response)


# if __name__ == "__main__":
#     uvicorn.run("main:app", host="0.0.0.0",port=8002)


import pandas as pd
from pymongo import MongoClient
from name_matching.name_matcher import NameMatcher

# 1️⃣ Connect to MongoDB
client = MongoClient("")
db = client["AML_EXIM_BANK"]
collection = db["Adverse_media_4"]


# 1️⃣ Load all persons from MongoDB
raw_name_data = [doc.get('persons') for doc in collection.find({}, {'persons': 1, '_id': 0}) if doc.get('persons')]

# 2️⃣ Flatten the list (assuming each 'persons' is a list)
flat_name_data = [name for sublist in raw_name_data for name in sublist]

# 3️⃣ Remove duplicates and empty strings
flat_name_data = [name.strip() for name in flat_name_data if name and name.strip()]
flat_name_data = list(set(flat_name_data))

# 4️⃣ Convert to DataFrame for NameMatcher
name_df = pd.DataFrame(flat_name_data, columns=['persons'])

# 5️⃣ Initialize NameMatcher
matcher = NameMatcher(
    top_n=10,
    lowercase=True,
    punctuations=True,
    remove_ascii=True,
    legal_suffixes=False,
    common_words=False,
    verbose=True
)

# 6️⃣ Set distance metrics
matcher.set_distance_metrics([
    'discounted_levenshtein',
    'SSK',
    'fuzzy_wuzzy_token_sort'
])

# 7️⃣ Load and process master data
matcher.load_and_process_master_data('persons', name_df)

# 8️⃣ Prepare user queries as the same DataFrame (matching names in DB)
user_query = pd.DataFrame(flat_name_data, columns=['query'])

# 9️⃣ Match each name against the database
results = matcher.match_names(to_be_matched=user_query, column_matching='query')

# 🔟 Print results
for r in results:
    print(r)

